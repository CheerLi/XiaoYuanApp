...
public class AsyncHttpPost {
	...
	AsyncHttpPost(OnResponseListener mOnResponseListener,String iName) {
		this.client = SingleAsyncClient.getSingleClient();
		this.responseHandler = new HttpResponseHandler(mOnResponseListener,iName);
	}
	 ...
	 */
	public AsyncHttpPost(String iName, OnResponseListener mOnResponseListener,
			String user_id, String insert_count, List<String> picList, String del_pictures,
			String name, String moto, String affective) {
		this(mOnResponseListener, iName);
		switch(iName){
			case "updateinfo":
				this.url = url + "/yaf/index.php/Userinfo/updateinfo";
				this.params = HttpRequestParams.updateinfo(user_id, insert_count, picList, del_pictures, name, moto, affective);
